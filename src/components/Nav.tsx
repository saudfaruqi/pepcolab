...
const ChevronDown = ({ rotated }: { rotated: boolean }) => (
  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
    style={{ transition: 'transform .2s', transform: rotated ? 'rotate(180deg)' : 'rotate(0deg)', opacity: .45 }}>
...
export default function Nav() {
  ...
  useEffect(() => {
    const onResize = () => { if (window.innerWidth >= 1024) { setMobileOpen(false); setMobileExpanded(null) } }
  ...
  const S = {
    ...
    logo: {
      ...
      color: '#0d0d0d', textDecoration: 'none', flexShrink: 0, letterSpacing: '-.02em',
    } as React.CSSProperties,
    navLink: {
      display: 'flex', alignItems: 'center', gap: 4,
      padding: '8px 12px', fontSize: 13.5, fontWeight: 500,
      color: 'rgba(13,13,13,.6)', borderRadius: 8, border: 'none', background: 'none',
    ...
    iconBtn: {
      ...
      cursor: 'pointer', color: 'rgba(13,13,13,.6)', position: 'relative' as const,
      transition: 'background .15s, color .15s',
    } as React.CSSProperties,
    shopBtn: {
      ...
      padding: '9px 20px', borderRadius: 40, border: '1.5px solid #0d0d0d',
      cursor: 'pointer', textDecoration: 'none', whiteSpace: 'nowrap' as const,
    } as React.CSSProperties,
    searchBtn: {
      display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, fontWeight: 500,
      color: 'rgba(13,13,13,.45)', border: '1px solid rgba(13,13,13,.12)',
      padding: '8px 14px', borderRadius: 8, background: 'none', cursor: 'pointer',
      whiteSpace: 'nowrap' as const, transition: 'border-color .15s, color .15s',
    } as React.CSSProperties,
    dropdown: {
      position: 'absolute' as const, top: '100%', left: 0, paddingTop: 8, zIndex: 60,
    } as React.CSSProperties,
    dropPanel: {
      background: '#fff', border: '1px solid rgba(13,13,13,.09)',
      borderRadius: 16, overflow: 'hidden', minWidth: 230,
      boxShadow: '0 12px 40px rgba(13,15,20,.12)',
      padding: '8px 0',
    } as React.CSSProperties,
    dropItem: {
      display: 'flex', flexDirection: 'column' as const,
      padding: '10px 16px', textDecoration: 'none', transition: 'background .12s',
    } as React.CSSProperties,
  }
  ...
  return (
    <>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', borderBottom: '1px solid rgba(13,13,13,.08)' }}>
              ...
              <input
                ...
                style={{
                  flex: 1, fontSize: 14, outline: 'none', border: 'none', background: 'transparent',
                  color: '#0d0d0d', minWidth: 0,
                }}
              />
              {searchQuery ? (
                <button onClick={() => setSearchQuery('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(13,13,13,.4)', padding: 4, display: 'flex' }}>
                  <CloseIcon size={14}/>
                </button>
              ) : (
            ...
            {searchResults.length > 0 ? (
              <div style={{ paddingBottom: 8 }}>
                ...
                <div style={{ padding: '10px 20px', borderTop: '1px solid rgba(13,13,13,.07)' }}>
                  ...
                  >
                    View all results for "{searchQuery}" <ArrowRight/>
                  </a>
                </div>
              </div>
              ...
              <div style={{ padding: '16px 20px 20px' }}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(13,13,13,.35)', marginBottom: 12 }}>Popular searches</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {['BPC-157','GLP-1','TB-500','Cognitive','Recovery','Anti-ageing'].map(q => (
                    <button key={q} onClick={() => setSearchQuery(q)} style={{ fontSize: 12, color: 'rgba(13,13,13,.6)', border: '1px solid rgba(13,13,13,.12)', padding: '6px 14px', borderRadius: 40, background: 'none', cursor: 'pointer', transition: 'border-color .15s, color .15s' }}
      ...
      <div style={{
        ...
        padding: '9px 16px', letterSpacing: '.02em', lineHeight: 1.5,
      }}>
        🧪&nbsp; Free UK tracked shipping on orders over £80 &nbsp;·&nbsp; HPLC-verified, Eurofins-tested &nbsp;·&nbsp;
        <a href="/certificates" style={{ color: 'rgba(255,255,255,.7)', textDecoration: 'underline', textUnderlineOffset: 2 }}>View all COAs →</a>
      </div>
      ...
      <nav style={S.nav}>
        <div style={S.inner}>
                    ...
                    <div style={S.dropPanel}>
                      {link.items.map(item => (
                        ...
                        >
                          ...
                          {item.sub && <span style={{ fontSize: 11, color: 'rgba(13,13,13,.4)', marginTop: 2 }}>{item.sub}</span>}
                        </a>
                      ))}
                    </div>
                  </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            ...
            >
              ...
              <kbd style={{ fontSize: 10, fontWeight: 600, border: '1px solid rgba(13,13,13,.12)', padding: '2px 6px', borderRadius: 5, lineHeight: 1.4 }} className="nav-search-kbd">⌘K</kbd>
            </button>
            ...
            >
              ...
              {totalQuantity > 0 && (
                <span style={{
                  ...
                  minWidth: 17, height: 17, background: '#0d0d0d', color: '#fff',
            ...
            <a href="/products" style={S.shopBtn} className="nav-shop-btn">Shop Now</a>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', height: 64, borderBottom: '1px solid rgba(13,13,13,.08)', flexShrink: 0 }}>
            <a href="/" style={S.logo} onClick={() => setMobileOpen(false)}>
              Pepco<em style={{ fontStyle: 'italic', color: 'rgba(13,13,13,.35)' }}>Lab</em>
            </a>
            <button onClick={() => setMobileOpen(false)} style={{ ...S.iconBtn }}>
              <CloseIcon/>
            </button>
          </div>
          ...
          <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(13,13,13,.07)' }}>
            <button
              ...
              style={{
                ...
                padding: '12px 16px', border: '1px solid rgba(13,13,13,.12)', borderRadius: 10,
                background: '#f5f5f3', cursor: 'pointer', textAlign: 'left' as const,
              }}
            >
              <SearchIcon/>
                  ...
                  <>
                    <button
                      ...
                      style={{
                        ...
                        padding: '16px 20px', fontSize: 15, fontWeight: 600, color: '#0d0d0d',
                      ...
                      <div style={{ background: '#f9f9f8', borderTop: '1px solid rgba(13,13,13,.06)', paddingBottom: 8 }}>
                        {link.items.map(item => (
                          ...
                          >
                            ...
                            {item.sub && <span style={{ fontSize: 11.5, color: 'rgba(13,13,13,.42)', marginTop: 2 }}>{item.sub}</span>}
                          </a>
                        ))}
                      </div>
                    )}
      ...
      <style>{`
        /* Desktop: show search + shop btn, hide hamburger */
        ...
        /* Tablet: hide search label + kbd, show hamburger */
